...
interface SunWordpressService {
    ...
    @GET("wp/v2/posts")
    fun getPostsById(@Query("page") page: Int): Call<List<Post?>?>?
    ...
    fun getPostsByAuthor(@Path("authorId") authorId: Int): Call<List<Post?>?>?

    @GET("wp/v2/media/{mediaId}")
    fun getMedia(@Path("mediaId") mediaId: Int): Call<List<Post?>?>?
